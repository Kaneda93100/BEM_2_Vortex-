...
def main():
    ...
    for pct in DATA_PCTS:
        ...
        for e, r, i, has_ae, opt in test_models:
            ...
            model_base_name = f"{e}_{r}_{i}_{ae_label}_{opt}_P{pct}"
            ...
            t0 = time.perf_counter()
            optimize(df_train_pct, entree=e, residuelle=r, inter=i, has_ae=has_ae, option=opt, model_base_name=model_base_name, n_trials=n_trials_current)
            print(f"   [CHRONO] optimize {model_base_name} : {time.perf_counter()-t0:.1f}s")

            t0 = time.perf_counter()
            evaluator(df_train_pct, df_test, entree=e, residuelle=r, inter=i, has_ae=has_ae, option=opt, baseline_scores=baseline_scores, pct=pct)
